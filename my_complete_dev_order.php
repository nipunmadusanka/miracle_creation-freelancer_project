...
<head>
  <meta charset="utf-8">
  <title>My Development Order || <?php  echo $row['name'];?></title>
...
<div class="main">
  <div class="inner" style="padding-top: 10em;">
     <center><h2>My Development Order</h2> </center> 
    <div class="thumbnails">
      <div class="content">
       <div class="thumbnails1" style="width: 70%;background-color: white;">

      <?php 
$ret=mysqli_query($con," SELECT * FROM  dev_order WHERE agent_read ='read' && user_id='$cid'");
while ($row=mysqli_fetch_array($ret)) {
?>
...
<p>
  Order Reasen :
<i style="color: white;"><?php  echo $row['reasen'];?></i>
</p>
<p>
  Order Status :
...
<div class="box2" >
 ...
 $msg=$_POST['msg'];
 $id=$_POST['id'];
$query=mysqli_query($con,"update dev_order set msg='$msg' where agent_read ='read' && user_id='$cid' && ID='$id'");
if($query){
...
<form action="" method="post" enctype="multipart/form-data" class="form-horizontal">
  ...
  <button type="submit" name="submit" title="send">Send</button>
        ...
        <?php 
$ret=mysqli_query($con," SELECT * FROM dev_order WHERE agent_read ='read' && user_id='$cid' && ID='$id'");
...
<br><br><p>Show Message For Selected Id</p>
 ...
 $id1=$_POST['id1'];
$ret=mysqli_query($con," SELECT * FROM dev_order WHERE agent_read ='read' && user_id='$cid' && ID='$id1'");
if ($row=mysqli_fetch_array($ret)) {
?>
<i>Order Id:</i>
