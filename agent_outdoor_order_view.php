...
if(isset($_GET['ok']))
  {
$ID=$_GET['ok'];
$que=mysqli_query($con,"update outdoor_order set agent_read='read' where ID='$ID' ");
if ($que) {
...
include_once('include/agent_menubar.php');
 ?>
    <div id="main">
          ...
          <div class="inner">
              <center><h2>Uncomplete Outdoor Orders</h2></center>
              <div class="box wow fadeInUp" style="padding-top: 5em;">
            <table border="1" class="mg-b-0" style="background-color: gray;">
            	    <?php
 // $eid=$_SESSION['adid'];
$ID=$_GET['id'];
$ret=mysqli_query($con,"select * from outdoor_order where ID ='$ID' && agent_id='$eid'");
$cnt=1;
while ($row=mysqli_fetch_array($ret)) {
	?>
<tr>
...
<tr>
 ...
 <tr>
 	<th></th>
 <td>     <!--  -->
<a href="agent_outdoor_order_view.php?ok=<?php echo $row['ID'];?>"><button style="background-color: rgba(100, 30, 250, 0.7);width: 7em;"title="complete">Complete</button></a>
</td>
</tr>
<?php  } ?>
</table>
